...
class F1LiveState:
    ...
    # (UI/Logs nutzen das bereits als "Inter/Wet share")
...
class F1UDPListener:
    ...
    def _run(self):
        ...
        while not self._stop.is_set():
            ...
            if pid == 1:
                ...
                # --- Rain: current + forecast ---
                rain_now_raw = data[base + 7]  # m_rainPercentage

                # --- Forecast from structured block (no scan) ---
                rain_fc_raw = None
                fc_series = []
                ...
                # Rain NOW
                if 0 <= rain_now_raw <= 100:
                    r_now = self._deb_rain_now.update(int(rain_now_raw))
    ...
    def _update_field_metrics_and_emit(self):
        ...
        print("[DELTA DEBUG] percar_deltas", len(deltas),
              "field_delta", self.state.pace_delta_inter_vs_slick_s)
